...
func TestManagerUpCleansUpStaleStateAndSucceeds(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(m.statePath("demo")), 0o700); err != nil { t.Fatal(err) }
	if err := writeState(m.statePath("demo"), state); err != nil { t.Fatal(err) }

	got, err := m.Up(context.Background(), UpOptions{Name: "demo", SSHTarget: "bob@host", MirrorPorts: []int{8080}})
	if err != nil { t.Fatal(err) }
	if got.SSHTarget != "bob@host" { t.Fatalf("Up() target = %q", got.SSHTarget) }
	if len(runner.checkCalls) != 1 { t.Fatalf("Check calls = %d, want 1", len(runner.checkCalls)) }
	if len(runner.upCalls) != 1 { t.Fatalf("Up calls = %d, want 1", len(runner.upCalls)) }
	if _, statErr := os.Stat(state.ControlSocket); !errors.Is(statErr, os.ErrNotExist) { t.Fatalf("stale socket still exists: %v", statErr) }
}
...
func TestManagerUpPreservesStateOnNonStaleCheckError(t *testing.T) {
	...
	if err := os.MkdirAll(filepath.Dir(m.statePath("demo")), 0o700); err != nil { t.Fatal(err) }
	if err := writeState(m.statePath("demo"), state); err != nil { t.Fatal(err) }
	if err := os.WriteFile(state.ControlSocket, []byte("socket"), 0o600); err != nil { t.Fatal(err) }
	...
	if _, err := m.load("demo"); err != nil { t.Fatalf("state should remain: %v", err) }
	if len(runner.upCalls) != 0 { t.Fatalf("unexpected Up calls: %#v", runner.upCalls) }
}
...
func TestManagerEnsureMirrorSelfHealsStaleSessionAndReusesMapping(t *testing.T) {
	...
	mapping, reused, err := m.EnsureMirror(context.Background(), "demo", 8080)
	if err != nil { t.Fatal(err) }
	if !reused || mapping.LocalPort != 8080 { t.Fatalf("EnsureMirror() = %#v, %v", mapping, reused) }
	if len(runner.upCalls) != 1 { t.Fatalf("expected recovery Up once, got %d", len(runner.upCalls)) }
	if len(runner.forwardCalls) != 1 || runner.forwardCalls[0].LocalPort != 43210 || runner.forwardCalls[0].RemotePort != 9090 { t.Fatalf("forward replay = %#v", runner.forwardCalls) }
	if runner.upCalls[0].MirrorPorts == nil || len(runner.upCalls[0].MirrorPorts) != 1 || runner.upCalls[0].MirrorPorts[0] != 8080 { t.Fatalf("recovery mirrors = %#v", runner.upCalls[0].MirrorPorts) }
}
...
func TestManagerEnsureMirrorDoesNotRecoverOnContextDeadline(t *testing.T) {
	...
	if len(runner.upCalls) != 0 { t.Fatalf("unexpected recovery Up calls: %#v", runner.upCalls) }
	if _, err := m.load("demo"); err != nil { t.Fatalf("state should remain: %v", err) }
}
...
func TestManagerEnsureMirrorSelfHealDefaultsMissingStateFields(t *testing.T) {
	...
	if _, _, err := m.EnsureMirror(context.Background(), "demo", 8080); err != nil { t.Fatal(err) }
	if len(runner.upCalls) != 1 { t.Fatalf("expected recovery Up once, got %d", len(runner.upCalls)) }
	if runner.upCalls[0].RemoteBobPort != DefaultRemoteBobPort { t.Fatalf("remote bob port = %d, want %d", runner.upCalls[0].RemoteBobPort, DefaultRemoteBobPort) }
	if runner.upCalls[0].LocalBobdAddr != DefaultLocalBobdAddr { t.Fatalf("local bobd addr = %q, want %q", runner.upCalls[0].LocalBobdAddr, DefaultLocalBobdAddr) }
}
...
func TestManagerEnsureMirrorReplaysFallbackLocalPortExactly(t *testing.T) {
	...
	if _, _, err := m.EnsureMirror(context.Background(), "demo", 9090); err != nil { t.Fatal(err) }
	if len(runner.forwardCalls) != 1 { t.Fatalf("forward calls = %d", len(runner.forwardCalls)) }
	if got := runner.forwardCalls[0]; got.LocalPort != 43210 || got.RemotePort != 9090 { t.Fatalf("forward replay = %#v", got) }
}
...
func TestManagerEnsureMirrorRollsBackOnRecoveryFailure(t *testing.T) {
	...
	if _, _, err := m.EnsureMirror(context.Background(), "demo", 9090); err == nil { t.Fatal("expected error") }
	if len(runner.downCalls) != 1 { t.Fatalf("Down calls = %d, want 1", len(runner.downCalls)) }
	if _, err := m.load("demo"); err != nil { t.Fatalf("state should remain after failed recovery: %v", err) }
}
